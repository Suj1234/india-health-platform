              ...
              <div className="px-8 pt-6 pb-5 border-b border-border">
                <div className="flex items-start justify-between gap-6">
                  <div className="flex items-center gap-3">
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 240, damping: 18, delay: 0.1 }} className="flex h-10 w-10 items-center justify-center rounded-full bg-emerald-100 shrink-0">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground mt-0.5">Your health insurance is now active. A copy has been sent to your registered email.</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 bg-primary-50 border border-primary-200 rounded-xl px-4 py-2.5 shrink-0">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold font-mono text-primary-900 tracking-wider">{policy.policy_number}</p>
                    </div>
                    <button type="button" onClick={copyPolicyNumber} className="ml-1 p-1.5 rounded-lg hover:bg-primary-100 transition-colors text-primary-600" title="Copy policy number">
                      {copied ? <Check className="h-4 w-4 text-emerald-600" strokeWidth={2.5} /> : <Copy className="h-4 w-4" />}
                    </button>
                  </div>
                </div>
              </div>
                ...
                <div className="px-8 py-6">
                  ...
                  <div className="rounded-xl overflow-hidden border border-border mb-4">
                    ...
                    <div className="bg-emerald-50 border-t border-emerald-100 px-5 py-2 flex items-center gap-2">
                      ...
                      <p className="text-xs font-semibold text-emerald-700">Active · {policy.policy_start_date} – {policy.policy_end_date}</p>
                    </div>
                  </div>
                  <div>
                    {[
                      ...
                      <div key={label} className={cn('flex items-start gap-3 py-3', i < arr.length - 1 && 'border-b border-border/60')}>
                ...
                <div className="px-8 py-6 flex flex-col gap-5">
                  ...
                  <div className="rounded-xl bg-amber-50 border border-amber-200 px-4 py-3">
                    <p className="text-xs text-amber-700">
                      <strong>Free-look period:</strong> Not satisfied? Return within <strong>15 days</strong> for a full refund (before {policy.free_look_period_expires}).
                    </p>
                  </div>
                  <div className="flex flex-col gap-3 flex-1 justify-end">
                    {policy.policy_document_url ? (
                      <a href="/api/policy/download" download={`${policy.policy_number}.pdf`} className="block">
                        <Button size="lg" className="w-full" leftIcon={<Download className="h-4 w-4" />}>Download Policy PDF</Button>
                      </a>
                    ) : (
                      <Button size="lg" className="w-full" leftIcon={<Download className="h-4 w-4" />} disabled>Policy PDF — sending to email</Button>
                    )}
                    <div className="grid grid-cols-2 gap-3">
                      <div className="flex items-center gap-2.5 rounded-xl bg-muted/50 border border-border px-3 py-2.5">
                        ...
                        <div>
                          ...
                          <p className="text-xs font-medium text-foreground break-all">{policy.email || 'your email'}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2.5 rounded-xl bg-muted/50 border border-border px-3 py-2.5">
                    ...
                    <p className="text-center text-[10px] text-muted-foreground/60 leading-relaxed">
                      Insurance is the subject matter of solicitation · IRDAI Reg. No. 142
                      <br />
