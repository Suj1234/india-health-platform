# CLAUDE.md — India Health Insurance Platform
...
| Step | URL | Screen | Key Actions |
|------|-----|--------|-------------|
| 1 | `/apply/1` | Mobile OTP | Enter mobile → receive & verify OTP |
...
| Cover Type | `cover_type` | `proposer_is_insured` | Insured Members |
|---|---|---|---|
| Individual | `individual` | `true` | Proposer only |
| Family Floater | `family_floater` | `true` | Proposer + spouse + up to 4 children |
| Parents Plan | `parents` | `false` | Father and/or Mother (NOT proposer) |

**Parents Plan rules:**
- Proposer is the adult child — they pay, sign, and complete KYC — but are **not** an insured member
- Health declaration is per insured member only (proposer health is irrelevant)
- NuralX vitals scan: skip OR send separate mobile link to each parent
- Quote is based on parents' ages, not proposer's age
- KYC: Proposer → Aadhaar (front+back) + selfie (Karza face match) + PAN; Father → Aadhaar (front+back) only; Mother → Aadhaar (front+back) only
- 80D tax: parents' premium is separately deductible — that is why this is a distinct cover type, not a family floater variant

**NuralX is health vitals (rPPG face scan), NOT face matching.** It measures HR, BP, SpO2, respiratory rate, and stress index. Face matching (selfie vs Aadhaar photo) is done by Karza OCR — separate step, always proposer only.
...
9. **Medical questionnaire is mandatory** for all insured members (not proposer if `proposer_is_insured = false`).
10. **Minimum KYC**: Aadhaar + PAN mandatory for proposer. Insured members who differ from proposer need Aadhaar (age proof). Additional docs based on sum insured threshold.
11. **Single plan per insurer** — no 3-tier Basic/Standard/Premium. Riders are the personalisation layer.
12. **`cover_type`** must be set at Step 2 and never changed mid-journey.
...
initiated
  → otp_verified
    → profiling_done        (iAdore complete)
      → members_added       (cover type + insured members saved at Step 2)
        → medical_done      (per-member health declaration + NuralX at Step 3)
          → quote_selected  (single plan + riders confirmed at Step 4)
            → proposal_submitted  (proposal + nominee at Step 5)
              → docs_uploaded     (KYC documents at Step 6)
                → stp_evaluated
...
Removed statuses (old flow, no longer used): `income_done`, `needs_done`, `biometrics_done`, `id_verified`
...
- Do not show health questions for the proposer when `proposer_is_insured = false` (parents plan)
- Do not run NuralX on the proposer when `proposer_is_insured = false`
- Do not use 3-tier plan selection (Basic/Standard/Premium) — single plan only
- Do not change `cover_type` after Step 2 is submitted
